class PlenumMinuteMailer < ActionMailer::Base
  ...
  def send_draft minute, user
    ...
    mail(from: get_email_with_name(user), subject: "Protokoll der Vollversammlung vom #{l @minutes_minute.date} [ENTWURF]")
  end
  def publish minute, user
    @minutes_minute = minute
    mail(from: get_email_with_name(user), subject: "Protokoll der Vollversammlung vom #{l @minutes_minute.date}")
  end
